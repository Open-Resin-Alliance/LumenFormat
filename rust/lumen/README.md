...
The corpus itself is generated by `make_vectors.py` and checked by
`verify_vectors.py`, which share no code with each other - and `cross_check.py`
points that second one at files *this crate* wrote. Agreement between all three is
evidence about the specification, not self-consistency, and CI runs all of it on
every change: the corpus under the specification's own reader, this crate's output
under that reader, this crate's suite over the corpus, plus formatting, lints, the
declared minimum Rust version, the `wasm32` target and the packaging path a release
takes.
