...
/// The decompressed-size bound a block of `layers` layers may not exceed.
///
/// Grayscale REE costs at most about 5 bytes per pixel plus framing, which is
/// what a reader checks a `block_table` entry against before allocating
/// (section 11.3).
...
mod tests {
    ...
    fn zstd_round_trips_with_and_without_a_dictionary() {
        ...
        assert_eq!(err.check(), Check::LayrBlockDecompressedSize);
    }
