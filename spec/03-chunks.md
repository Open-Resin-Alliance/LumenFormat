...
([§5](08-layer-encoding.md#5-layer-mask-encoding)), where `0x00` is black - no exposure - `0xFF`
...
- **Mirroring.** `mirror_x` / `mirror_y` are build settings the slicer resolves before encoding.
  ...
  ([§1.1](01-overview.md#11-what-lumen-is-not)).
- **Bit depth.** Every mask pixel is 8 bits, whatever the display can emit, and the file does not
  ...
  `printer.bit_depth` a `PROF` chunk ([§4.3](#43-prof---print-profile-chunk)) may carry describes
...
one ([§8](11-layer-timing.md#8-per-layer-settings-model)). When `material_index` is present
...
```jsonc
{
  ...
  "settings": {
    ...
    // Resin working curve - experimental ([§5.7](08-layer-encoding.md#57-resin-working-curve-experimental)). If omitted, printer falls back
